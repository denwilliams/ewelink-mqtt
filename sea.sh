#! /bin/bash
nodejs_version="${NODEJS_VERSION:-22.11.0}"
NODE_DIR=".sea/node"

mkdir -p "${NODE_DIR}"
npx esbuild main.mjs --bundle --platform=node --target=node22.11 --outfile=.sea/main.mjs
echo '{ "main": ".sea/main.mjs", "output": ".sea/prep.blob", "disableExperimentalSEAWarning": true }' > .sea/config.json
